...
//CREATING NEW TABLES
data.run(`CREATE TABLE TB_ALUNO (
    id INTEGER NOT NULL,
    name TEXT NOT NULL,
    birth TEXT NOT NULL,
...
data.run(`CREATE TABLE TB_DISCIPLINA (
    id INTEGER NOT NULL,
    name TEXT NOT NULL,
    PRIMARY KEY ("id" AUTOINCREMENT)
 )`);

data.run(`CREATE TABLE TB_PROFESSOR (
   id INTEGER NOT NULL,
   name TEXT NOT NULL,
   birth TEXT NOT NULL,
   PRIMARY KEY ("id" AUTOINCREMENT)
...
data.run(`CREATE TABLE TB_MATRICULA (
   id INTEGER NOT NULL,
...
data.run(`CREATE TABLE TB_PROFESSOR_DISCIPLINA (
    id INTEGER NOT NULL,
    disciplina_id INTEGER NOT NULL,
    professor_id INTEGER NOT NULL,
    PRIMARY KEY ("id" AUTOINCREMENT)
...
data.all("SELECT * FROM TB_ALUNO", function(err, rows) {
    rows.forEach(function (row) {
      console.log(row.id + ": " + row.name + row.birth);
    });
   });

data.all("SELECT * FROM TB_PROFESSOR", function(err, rows) {
    rows.forEach(function (row) {
      console.log(row.id + ": " + row.name + row.birth);
    });
   });
  
data.all("SELECT * FROM TB_DISCIPLINA", function(err, rows) {
    rows.forEach(function (row) {
