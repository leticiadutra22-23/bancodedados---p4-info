...
db.run(`CREATE TABLE IF TB_DISCIPLINA (
    id INTEGER NOT NULL,
    name TEXT NOT NULL,
    PRIMARY KEY ("id" AUTOINCREMENT)
 )`);

db.run(`CREATE TABLE TB_PROFESSOR (
   id INTEGER NOT NULL,
   name TEXT NOT NULL,
   birth TEXT NOT NULL,
   PRIMARY KEY ("id" AUTOINCREMENT)
...
db.run(`CREATE TABLE TB_MATRICULA (
   id INTEGER NOT NULL,
...
db.run(`CREATE TABLE TB_PROFESSOR_DISCIPLINA (
    id INTEGER NOT NULL,
    disciplina_id INTEGER NOT NULL,
    professor_id INTEGER NOT NULL,
    PRIMARY KEY ("id" AUTOINCREMENT)
...
db.run(`INSERT INTO TB_PROFESSOR_DISCIPLINA (disciplina_id, professor_id) VALUES (1, 1), (2, 2), (3, 2), (3, 3)`);


//QUERYING THE DATABASE
db.all("SELECT * FROM TB_ALUNO", function(err, rows) {
    rows.forEach(function (row) {
      console.log(row.id + ": " + row.name + row.birth);
    });
   });

db.all("SELECT * FROM TB_PROFESSOR", function(err, rows) {
    rows.forEach(function (row) {
      console.log(row.id + ": " + row.name + row.birth);
    });
   });
  
db.all("SELECT * FROM TB_DISCIPLINA", function(err, rows) {
    rows.forEach(function (row) {
